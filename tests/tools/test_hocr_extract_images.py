...
from tests import TestCase, chdir
...
class HocrExtractImagesTestCase(TestCase):
    def _check(self, *, level: str, basename: str, expected_count: int, stdin: bool, extra_args: str = "") -> None:
        with TemporaryDirectory() as temp_directory:
            ...
            if stdin:
                command = [
                    f'cat "{tess_hocr!s}" | hocr-extract-images{extra_args} -p "{basename}-%03d.png" -b "{directory!s}" -e "{level}"'
                ]
            else:
                command = [
                    "hocr-extract-images", extra_args, "-p", f"{basename}-%03d.png", "-b", str(directory), "-e", level, str(tess_hocr)
                ]
    ...
    def test_main(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            command = [
                "hocr-extract-images", "-p", "word-%03d.png", "-b", directory, "-e", "ocrx_word", str(tess_hocr)
            ]
